...
| C080 | Optional Hybrid integration with default off | IN_PROGRESS | Minimal staged plan is frozen after independent review with 0 Blocker/High. The sidecar/descriptor/ready/header draft was rejected; the accepted force-v1 uses grouped static slots, a 4-byte transit key, existing Tag0, and one post-unshuffle barrier. No Hybrid source edit has started. |
| C080-A | Contract, baseline fingerprints, and off identity | IN_PROGRESS | Legacy goldens pass 4/4, planner tests pass 38/38, and default-off EP8 FP8 smoke passes. Next is backward-compatible constructor/config plus a disabled force capability; old JIT inputs, bytes, runtime selection, and off handles remain unchanged. |
| C080-B | GPU count, plan, and grouped proxy schedule | PLANNED | Real `topk_idx` produces channel-major counts, minimum-move segments, and descriptor-free `(egress,channel,destination)` group prefixes matching an independent CPU oracle; the C061 fixture produces 33 copies and exactly six moves without a test manifest or full per-copy assignment table. |
| C080-C | Minimal force arena and collective preflight | PLANNED | Only symmetric count/control plus dispatch/return payload arenas are reserved. Mismatched/unsupported/capacity/returned-JIT errors make all ranks exit before publication; public force on physical scaleout=1 reports `UnsupportedTopology`. |
...
| C090 | Exhaustive local correctness, sanitizer, and fault campaign | PLANNED | Local planner/materializer/API/topology/state/fault spaces are exercised as broadly as practical; required tools are clean or every limitation is explicitly classified. |
