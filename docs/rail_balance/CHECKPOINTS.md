...
| C080-D | Shared-core LSA data path without a supplied manifest | IN_PROGRESS | Direct-final source shuffle plus static return-unshuffle pass true 8-GPU H256/H7168 and D32>K4 raw TokenLayout comparison, exact owner row/token routing, poison-preserved non-targets, exact Pcap, corruption/recovery, and non-default streams. The two adapter kernels and exact legacy-epilogue wrapper compile for H256/K4 and H7168/K8 with zero spills and pass independent post-fix audits; one retained identity hole, one status-boundary bug, and one partial-return state transition were found and closed. CPU remote-only fixtures now distinguish two-level BF16 rounding and cover nonzero moved/group prefixes; the 4x2/2x4/N=0 GPU round trips remain acceptance gates. Focused source mem/sync/device-init checks report zero errors; racecheck retains six explained WAW warnings. |
...
| C080-F | Isolated Hybrid combine plus return-unshuffle | IN_PROGRESS | The production-shared return-unshuffle passes true 8-GPU H256/H7168 and D32>K4 byte-exact tests, derives collision-free owner/row/token from group p plus preserved dispatch, and compiles at REG60/STACK72/LOCAL0 with no spill. A private source hook now prepares and launches the byte-identical legacy epilogue cache entry after unshuffle and passes static/state audit; vnode GPU execution and destination-side force-combine generation remain. |
...
| C090 | Exhaustive local correctness, sanitizer, and fault campaign | IN_PROGRESS | B1 memcheck/synccheck and the true 8-GPU source/fault transaction's filtered memcheck, synccheck, and device-side initcheck report zero errors. Racecheck reports zero errors plus six explained metadata WAW warnings. Return-unshuffle/combine, broader fault injection, and final sanitizer closure remain. |
