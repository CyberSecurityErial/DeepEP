...
| C080-B | GPU count, plan, and grouped proxy schedule | IN_PROGRESS | B1 private stacked-owner GPU materializer matches 69 exact H200 cases: C061, Pcap 3/2, zero token, 64 random seeds, C1024/D32, int64 seed, strict route rejection, and non-default stream. Three cubins each export one symbol; no manifest/descriptor/ring exists. B2 per-rank LSA count snapshot remains. |
| C080-C | Minimal force arena and collective preflight | IN_PROGRESS | The checked 32B/2MiB force arena formula and EP8 size agreement pass. Private prepare/Gate1/local-only LSA snapshot/plan/Gate2 transaction is frozen after 0 Blocker/High review; implementation and fault convergence remain. Public force stays unavailable. |
