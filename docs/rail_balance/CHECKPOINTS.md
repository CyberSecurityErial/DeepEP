...
| C080-D | Shared-core LSA data path without a supplied manifest | IN_PROGRESS | Direct-final source shuffle plus static return-unshuffle pass true 8-GPU H256/H7168 and D32>K4 raw TokenLayout comparison, exact owner row/token routing, poison-preserved non-targets, exact Pcap, corruption/recovery, and non-default streams. Focused source mem/sync/device-init checks report zero errors; racecheck retains six explained WAW warnings. The 4x2/2x4 GPU round trips remain the acceptance gate. |
