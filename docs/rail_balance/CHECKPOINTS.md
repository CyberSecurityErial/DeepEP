...
| C080-D | Shared-core LSA data path without a supplied manifest | IN_PROGRESS | Minimal implementation is frozen as one warp/channel, compact device resolver, single shared TokenLayout staging, and direct-final moved-only LSA/TMA stores with a four-byte transit key. 8x1 exact bytes and 4x2/2x4 loops remain acceptance gates. |
