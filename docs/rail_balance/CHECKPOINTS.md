...
| C080-D | Shared-core LSA data path without a supplied manifest | IN_PROGRESS | Direct-final source shuffle plus static return-unshuffle pass true 8-GPU H256/H7168 and D32>K4 raw TokenLayout comparison, exact owner row/token routing, poison-preserved non-targets, exact Pcap, corruption/recovery, and non-default streams. The two adapter kernels and exact legacy-epilogue wrapper now compile for H256/K4 and H7168/K8 with zero spills and pass an independent post-fix audit; one retained identity hole and one status-boundary bug were found and closed. CPU remote-only fixtures pass structurally, while rounding-sensitive and nonzero moved-prefix coverage plus the 4x2/2x4 GPU round trips remain acceptance gates. Focused source mem/sync/device-init checks report zero errors; racecheck retains six explained WAW warnings. |
